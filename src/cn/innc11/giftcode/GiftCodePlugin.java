...
//        loadGiftsConfig();
        loadGiftCodesConfig();
        getLogger().info(TextFormat.colorize("加载 礼包: &e" + gifts.size()));
        getLogger().info(TextFormat.colorize("加载 GiftCodeSet: &e" + codes.size()));
    ...
    public void onEnable()
    {
        ...
        this.getLogger().info("§a礼包码插件加载成功，§e此为PNX专属版本,§d此版本维护者为XKSHADOW 和 Reiyans");
        //使用task保证在所有插件都加载完后为礼包添加物品
        //这是为了兼容PNX的自定义物品
        this.getServer().getScheduler().scheduleTask(this, () -> {
            this.getLogger().info("为礼包填充物品中");
            loadGiftsConfig();
        });
    }
